...
export default function StudentDataTable() {
    const [studentAccount, addAccount, deleteAccount, updateAccount] = useAccount();
    ...
    const [newAccount, setNewAccount] = React.useState({ id: 0, name: "", level: 0 });
    ...
    function add() {
        ...
        setNewAccount({ id: 0, name: "", level: 0 })
        setAddVisible(false)
        // console.log(account);
    }
    ...
    const columns: GridColDef[] = [
        { field: 'id', headerName: '學號', width: 150 },
        { field: 'name', headerName: '學生名字', width: 150 },
        {
            field: 'actions',
            headerName: '修改資料',
            width: 150,
            renderCell: (params) => (
                <Tooltip title="修改學生資料">
    ...
    const rows = studentAccount;

    return (
        <div style={{ height: 'auto', width: 'auto', margin: '50px' }}>
            <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="h6">學生名單</Typography>
                <div>
                    <Tooltip title="新增學生">
            ...
            {addVisible ?
                <Dialog open={addVisible} onClose={handleCloseClick} aria-labelledby="新增學生">
                    <DialogTitle>新增學生</DialogTitle>
                    <DialogContent>
                        <TextField label="學號" variant="outlined" name="id" value={newAccount.id} onChange={handleChange} /><p />
                        <br />
                        <TextField label="學生名字" variant="outlined" name="name" value={newAccount.name} onChange={handleChange} /><p />
                    </DialogContent>
                    <DialogActions>
                        <IconButton
                            aria-label="close"
            ...
            {editVisible ?
                <Dialog open={editVisible} onClose={handleCloseClick} aria-labelledby="修改學生資料">
                    <DialogTitle>修改學生資料</DialogTitle>
                    <DialogContent>
                        <TextField label="學號" variant="outlined" name="id" value={editAccount.id} onChange={handleChange} /><p />
                        <br />
                        <TextField label="學生名字" variant="outlined" name="name" value={editAccount.name} onChange={handleChange} /><p />
                    </DialogContent>
                    <DialogActions>
                        <IconButton
                            aria-label="close"
